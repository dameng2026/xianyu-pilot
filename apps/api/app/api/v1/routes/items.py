    ...
    try:
        ...
        async def remote_publish() -> RemoteOperationResult:
            ...
            try:
                ...
                # publisher 内部抛出的 RuntimeError 通常是图片下载/类目推荐等前置步骤失败，

                # 把异常消息带给前端，避免所有错误都显示"平台明确拒绝发布"

                return RemoteOperationResult.failed(

                    "platform_rejected",

                    str(exc) or "发布失败，请检查账号状态或商品资料后重试",
            ...
            if not isinstance(result, dict) or result.get("success") is not True:
                ...
                # publisher 已在 result.message 中给出具体原因（如违禁词、图片违规等），

                # 直接透传，避免前端只看到笼统的"平台明确拒绝发布"

                detail = (result.get("message") if isinstance(result, dict) else "") or "平台明确拒绝发布，请检查账号状态或商品资料后重试"
...
async def update_item_price(
    ...
    try:
        ...
        is_fish_shop = bool(getattr(account, "fish_shop_user", getattr(account, "fish_shop", False)))
        if not is_fish_shop:
...
async def republish_item(
    ...
    请求体：{ xianyuAccountId: int, itemId|xyGoodsId: str }
    放宽 original_quantity 与 status 限制，但仍要求 auto_relist_enabled=1 且 has_snapshot=1。
    """
    try:
        ...
        result = await manual_relist(account_id, external_goods_id)
        ...
        logger.error("手动重发失败", exc_info=True)
        return ResultObject.failed("重发失败，请稍后重试")
...
async def toggle_auto_relist(
    ...
    请求体：{ xianyuAccountId: int, itemId: str, enabled: bool }
    返回：{ enabled, hasSnapshot, message }
    开启时校验商品必须已有发布快照（has_snapshot=1）。
    """
    try:
        account_id = int(req.get("xianyuAccountId") or req.get("xianyu_account_id") or 0)
        if not account_id:
        ...
        logger.error("切换售整自动上架失败", exc_info=True)
        return ResultObject.failed("切换开关失败，请稍后重试")
...
async def _is_fish_shop_account(db: AsyncSession, account_id: int) -> bool:
    ...
    try:
        ...
        # 检查是否有 fish_shop_user 属性（模型扩展字段/实际列，兼容旧 fish_shop）

        return bool(getattr(account, "fish_shop_user", getattr(account, "fish_shop", False)))
...
async def polish_items(
    ...
    请求参数: { accountId: int }
    返回: { polished: int, failed: int, message: str }

    实现：遍历账号下未删除的在售商品，逐个调用闲鱼擦亮 API（mtop.taobao.idle.item.polish）。
    为避免长时间阻塞，最多擦亮前 50 个商品。同步执行，前端显示 loading。
    """
    try:
        account_id = req.get("accountId") or req.get("xianyuAccountId") or req.get("xianyu_account_id")
        if not account_id:
            return ResultObject.failed("缺少参数 accountId")
        account_id = int(account_id)
        ...
        # 查询在售商品（最多 50 个，避免长时间阻塞）
        goods_result = await db.execute(
            select(XianyuGoods.external_goods_id).where(
                XianyuGoods.account_id == account_id,
                XianyuGoods.deleted == 0,
                XianyuGoods.status == 1,
            ).limit(50)
        )
        goods_ids = [row[0] for row in goods_result.fetchall() if row[0]]
        if not goods_ids:
            return ResultObject.success({
                "polished": 0,
                "failed": 0,
                "message": "该账号暂无在售商品可擦亮",
            })
        ...
        for goods_id in goods_ids:
            try:
                # 调用闲鱼擦亮 API：mtop.taobao.idle.item.polish
                # 复用 operator._call_api 方法（已封装签名 + Cookie 注入 + 风控检测）
                operator._call_api(
                ...
                failed += 1
                errors.append(f"{goods_id}: {exc}")
                # 触发风控时立即终止，避免连续失败
                err_msg = str(exc)
                if "RGV587" in err_msg or "风控" in err_msg:
                    break

        message = f"擦亮完成：成功 {polished} 个，失败 {failed} 个"
        if failed and errors:
            message += f"；失败原因：{errors[0]}"

        logger.info(
            "商品擦亮完成 accountId=%d polished=%d failed=%d",
            account_id, polished, failed,
        )
        return ResultObject.success({
            "polished": polished,
            "failed": failed,
        ...
        logger.error("商品擦亮失败 errorType=%s", type(exc).__name__)
        return ResultObject.internal_error()
